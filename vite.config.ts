...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      watch: { ignored: ['**/.figma/**'] },
      // Allow the Figma preview proxy hosts through Vite's host check.
      // Without this, requests arriving via the proxy (e.g.
      // *.makeproxy-c.figma.site) are rejected with a blank
      // "Blocked request. This host is not allowed." page — the app
      // renders fine on localhost but appears blank inside the preview.
      allowedHosts: allowedHostsFromEnv(),
      // Disable HMR inside the Figma preview environment. The HMR
      // WebSocket cannot reach the dev server through the proxy, and its
      // failed reconnect attempts surface as the "WebSocket connection to
      // 'ws://…' failed" console errors. Local dev (no proxy) keeps HMR.
      hmr: !(process.env.FIGMA_MAKE || process.env.ALLOWED_HOSTS),
    },
    preview: {
      host: process.env.FIGMA_DEV_SERVER_HOST || '0.0.0.0',
      port: parseInt(process.env.PORT || '8443'),
...
/**
 * Builds Vite's `server.allowedHosts` list from the `ALLOWED_HOSTS`
 * environment variable the Figma preview provides (a comma-separated
 * list such as `.preview.figma.net,.makeproxy-c.figma.site`). A leading
 * dot allows every subdomain. When the variable is absent (plain local
 * dev) we return `true` to accept any host.
 */
function allowedHostsFromEnv(): string[] | true {
